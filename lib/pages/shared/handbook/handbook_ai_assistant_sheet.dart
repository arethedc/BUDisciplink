...
Future<void> showHandbookAiAssistantSheet(BuildContext context) async {
  ...
  if (isDesktop) {
    await showGeneralDialog<void>(
      ...
      barrierLabel: 'Close handbook AI',
...
class _HandbookAiAssistantSheetState extends State<_HandbookAiAssistantSheet> {
  ...
  final List<_Message> _messages = <_Message>[
    const _Message(
      fromUser: false,
      text:
          'Handbook AI is ready. Ask about policies, procedures, and handbook rules.',
      sources: <String>[],
    ),
  ];
  ...
  Future<void> _ask() async {
    ...
    setState(() {
      _messages.add(
        _Message(fromUser: true, text: question, sources: const []),
      );
      _loading = true;
      _input.clear();
    });
    ...
    try {
      ...
      setState(() {
        _messages.add(
          _Message(fromUser: false, text: answer.text, sources: answer.sources),
        );
      });
    } catch (error) {
      if (!mounted) return;
